...
set "AMENT_ENVIRONMENT_HOOKS[handler_pkg]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[handler_pkg] "%AMENT_CURRENT_PREFIX%\share\handler_pkg\environment\ament_prefix_path.bat"
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[handler_pkg] "%AMENT_CURRENT_PREFIX%\share\handler_pkg\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[handler_pkg]:;=";"%") do (
    call:call_file %%f
  )
